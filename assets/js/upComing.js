...
fetch("https://mindhub-xj03.onrender.com/api/amazing")
  ...
  .then((data) => {
    ...
    let eventsFilterByDate = data.events.filter( event => event.date > data.currentDate)
    renderCards(eventsFilterByDate, card);
    ...
    checkboxes.forEach((checkbox) => {
      checkbox.addEventListener("change", () => {
        ...
        filterCards(eventsFilterByDate, selectedCategories, searchInput, card);
      });
    });
    
    searchInput.addEventListener("keyup", () => {
      const searchQuery = searchInput.value.toLowerCase().trim();
      filterCards(eventsFilterByDate, selectedCategories, searchQuery, card);
    });
  })
